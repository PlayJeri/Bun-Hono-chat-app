...
export async function getConversationID(
  ...
  try {
    const query = await db.query(`
        ...
        `);
    const result = (await query.get({ $id1: id1, $id2: id2 })) as {
      id: string;
    };
...
export async function wsMessageHandler(
  ws: ServerWebSocket<WebSocketData>,
  server: Server,
  ...
  if (typeof message === "string") {
    ...
    switch (msg.type) {
      case "message":
        ...
        return;
        const { recipient, message } = msg.payload as {
          recipient: string;
          message: string;
        };
...
export async function wsConversationMessageHandler(
  ws: ServerWebSocket<WebSocketData>,
  server: Server,
